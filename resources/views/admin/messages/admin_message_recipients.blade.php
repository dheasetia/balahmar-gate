                    ...
                    @if(count($message->recipients) > 0)
                        @foreach($message->recipients as $user)
                            <tr>
                                ...
                                <td>
                                    <a href="{{url('admin/users', $user->id)}}" class="primary-link tooltips" data-original-title="تفاصيل المستخدم">{{$user->name}}</a>
                                </td>
                                @if(isset($user->office))
                                    <td><a href="{{url('admin/offices', $user->office->id)}}" class="primary-link tooltips" data-original-title="تفصيل الجهة">{{$user->office->name}}</a></td>
                                @else
                                    <td>---</td>
                                @endif
