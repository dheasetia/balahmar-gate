                        ...
                        <div class="col-md-12">
                            ...
                            <div class="portlet light portlet-fit portlet-form ">
                                <div class="portlet-title">
                                    <div class="caption">
                                        <i class=" fa fa-unlock font-balahmar"></i>
                                        <span class="caption-subject font-balahmar">استرجاع كلمة المرور</span>
                                    </div>
                                </div>
                                <div class="portlet-body">
                                    <!-- BEGIN FORM-->
                                    <form action="{{url('password/update')}}" method="post" id="update_password" class="form-horizontal" accept-charset="UTF-8">
                                        {{csrf_field()}}
                                        <div class="form-body">
                                            <h4>فضلا أدخل كلمة المرور الجديدة</h4>
                                            <div class="form-group {{$errors->has('password') ? 'has-error' : ''}}">
                                                <label for="password" class="control-label col-md-3">كلمة المرور الجديدة  <span class="required">*</span></label>
                                                <div class="col-md-9">
                                                    <input type="password" name="password" class="form-control" id="password" value="{{old('password')}}" autofocus/>
                                                    @if($errors->has('password'))
                                                        <div class="help-block help-block-error">{{$errors->first('password')}}</div>
                                                    @endif
                                                    @if(session('message'))
                                                        <div class="help-block help-block-error">{{session('message')}}</div>
                                                    @endif
                                                </div>
                                            </div>
                                            <div class="form-group {{$errors->has('password_confirmation') ? 'has-error' : ''}}">
                                                <label for="password_confirmation" class="control-label col-md-3">تأكيد كلمة المرور  <span class="required">*</span></label>
                                                <div class="col-md-9">
                                                    <input type="password" name="password_confirmation" class="form-control" id="password_confirmation" value="{{old('password_confirmation')}}" autofocus/>
                                                    @if($errors->has('password_confirmation'))
                                                        <div class="help-block help-block-error">{{$errors->first('password_confirmation')}}</div>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
...
<script src="{{asset('assets/layouts/layout3/scripts/update_password.js') }}" type="text/javascript"></script>
