        ...
        <div class="portlet light portlet-fit ">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-helper">تم الاعتذار عن موافقتها</span>
                </div>
            </div>
            <div class="portlet-body">
                <table class="table table-bordered table-striped" id="projects_table">
                    ...
                    <tbody>
                    @if(count($banned_projects) == 0)
                        <tr>
                            <td colspan="9" class="text-center"><h4>لا توجد مشاريع تعتذر المؤسسة عن الموافقة عليه</h4></td>
                        </tr>
                    @else
                        @foreach($banned_projects as $project)
                            <tr>
                                ...
                                <td>
                                    <a href="{{asset('files_projects') . '/' .$project->document_path}}" class="btn btn-icon-only btn-circle font-balahmar tooltips" data-original-title="مشاهدة ملف طلب المشروع"><i class="fa fa-file-pdf-o"></i></a>
                                    <a href="{{url('admin/projects', $project->id) . '/reports'}}" class="btn btn-icon-only btn-circle font-balahmar tooltips" data-original-title="{{count($project->reports) > 0 ? 'التقارير المرفوعة لهذا المشروع' : 'التقارير المرفوعة لهذا المشروع (لا يوجد تقرير)'}}"><i class="fa fa-pie-chart"></i></a>
                                </td>
                            </tr>
                        @endforeach
                    @endif
