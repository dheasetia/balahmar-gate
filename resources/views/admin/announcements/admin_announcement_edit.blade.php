        ...
        <div class="col-md-12">
            <div class="portlet light ">
                ...
                <div class="portlet-body form">
                    <form action="{{url('/admin/announcements', $announcement->id)}}" method="post" accept-charset="utf-8" enctype="multipart/form-data" id="form_announcement_create" role="form" class="form-horizontal form-bordered form-row-stripped">
                        {{csrf_field()}}
                        {{method_field('patch')}}
                        <div class="form-body form">
                            <div class="form-body">
                                <div class="form-group">
                                    ...
                                    <div class="col-md-10">
                                        <input type="text" name="hijri_created" class="form-control" id="hijri_created" value="{{$announcement->hijri_created_day . '/ ' . $announcement->hijri_created_month . '/ ' .$announcement->hijri_created_year}}" readonly/>
                                    </div>
                                </div>
                                <div class="form-group {{$errors->has('subject') ? 'has-error' : ''}}">
                                    <label for="subject" class="control-label col-md-2"> الموضوع  <span class="required">*</span></label>
                                    <div class="col-md-10">
                                        <input type="text" name="subject" class="form-control" id="subject" value="{{old('subject', $announcement->subject)}}"/>
                                ...
                                <div class="form-group {{$errors->has('body') ? 'has-error' : ''}}">
                                    ...
                                    <div class="col-md-10">
                                        <textarea id="summernote" class="form-control" name="body">{{old('body', $announcement->body)}}</textarea>
                                        @if($errors->has('body'))
                                                    ...
                                                    @if($user->id != Sentinel::getUser()->id)
                                                        <div class="mt-checkbox-list col-md-12">
                                                            <label for="user-{{$user->id}}" class="mt-checkbox mt-checkbox-outline"> {{$user->name}} (<strong>{{$user->office != null ? $user->office->name : '---'}}</strong>)
                                                                <input id="user-{{$user->id}}" value="{{$user->id}}" name="recipients[]" data-group="@foreach($user->groups as $user_group) {{'"' . $user_group->slug . '",'}} @endforeach" class="user_checkbox"  type="checkbox" {{in_array($user->id, $recipients) ? 'checked' : ''}}>
                                                                <span></span>
                                                            </label>
                                                        </div>
                                                    @endif
...
@section('page_scripts')
    <script src="{{asset('assets/layouts/layout3/scripts/admin_announcement_edit.js')}}"></script>
@endsection